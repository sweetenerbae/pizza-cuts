...
export default function Home() {
    ...
    try {
      Promise.resolve(context.registerTool({
        ...
        execute(input) {
          ...
          flushSync(()=>{setPeople(n);setValue(String(n));setCut(true)});
  ...
  const update = (n: number) => { const next = Math.max(1, Math.min(100,n)); setValue(String(next)); setPeople(next); setCut(true); };
    ...
    <main>
      ...
      <section className="calculator" aria-label="Калькулятор разреза пиццы">
        <div className="controls">
          ...
          <div className="receipt" aria-live="polite" aria-atomic="true"><div className="receipt-heading">ЧЕК СПРАВЕДЛИВОСТИ <span>✓</span></div><div className="stats"><div><strong>{cuts}</strong><span>{people===1?'разрезов':people%2===0?'сквозных разрезов':'надрезов от центра'}</span></div><div><strong>{fmt(angle)}<i>°</i></strong><span>угол кусочка</span></div><div><strong>1/{people}</strong><span>пиццы каждому</span></div></div><div className="receipt-bottom">{people===1?'Вся пицца твоя. И никаких переговоров.':people>20?'Это уже дегустация. Но зато честная.':'Никто не обделён. Математика обещает.'}<span>☺</span></div></div>
        </div>
      ...
      <p className="fine-print">Считаем для круглой пиццы и равных кусочков. За распределение пепперони ответственности не несём.</p>
